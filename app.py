...
def flatten_llm_result(llm_result: dict) -> dict:
    flat = {}
    for key in REQUIRED_KEYS:
        ...
        if isinstance(val, dict):
            ...
            if "full_name" in val:
                parts.append(val["full_name"])
            elif "name" in val:
                parts.append(val["name"])

            # адрес
            if "address" in val:
                parts.append(val["address"])

            # реквизиты
            if "requisites" in val and isinstance(val["requisites"], dict):
                inn = val["requisites"].get("inn")
                kpp = val["requisites"].get("kpp")
                if inn: parts.append(f"ИНН {inn}")
                if kpp: parts.append(f"КПП {kpp}")

            # vehicle.details или прямые поля
            if "details" in val and isinstance(val["details"], dict):
                model = val["details"].get("model")
                plate = val["details"].get("license_plate")
                if model: parts.append(model)
                if plate: parts.append(plate)
            if "model" in val:
                parts.append(val["model"])
            if "license_plate" in val:
                parts.append(val["license_plate"])
...
async def perform_ocr_with_llm(file: UploadFile = File(...)):
    ...
    flat_result = flatten_llm_result(llm_result)
    print(llm_result)

    return JSONResponse(content={
        "llmResult": flat_result
    })
